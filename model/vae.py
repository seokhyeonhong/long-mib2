...
class EncoderWithMask(nn.Module):
    def __init__(self, d_motion, config, is_context=True):
        super(EncoderWithMask, self).__init__()
        self.d_motion       = d_motion
        self.config         = config
        self.is_context     = is_context
        ...
        # encoders
        self.motion_encoder = nn.Sequential(
            nn.Linear(self.d_motion + 1, self.d_model), # +1 for mask
            nn.PReLU(),
            nn.Dropout(self.dropout),
            nn.Linear(self.d_model, self.d_model),
            nn.PReLU(),
    ...
    def forward(self, x, mask):
        ...
        # motion encoder
        mask = get_mask(x, self.config.context_frames)
...
class Decoder(nn.Module):
    def __init__(self, d_motion, config, is_context=True):
        ...
        # encoders
        self.motion_encoder = nn.Sequential(
            nn.Linear(self.d_motion + 1 + self.d_model, self.d_model), # (motion, mask, z)
            nn.PReLU(),
            nn.Dropout(self.dropout),
            nn.Linear(self.d_model, self.d_model),
            nn.PReLU(),
    ...
    def forward(self, motion, traj, z, mask=None):
        ...
        # encoders
        x = self.motion_encoder(torch.cat([motion, mask, z], dim=-1))
...
class VAEWithMask(nn.Module):
    def __init__(self, d_motion, config, is_context):
        super(VAEWithMask, self).__init__()

        self.d_motion   = d_motion
        self.config     = config
        self.is_context = is_context

        self.encoder = EncoderWithMask(d_motion, config, is_context=is_context)
        self.decoder = Decoder(d_motion, config, is_context=is_context)
